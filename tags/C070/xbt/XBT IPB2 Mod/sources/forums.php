...
class forums {
	...
	function render_forum()
	{
		...
		{
			$DB->query(sprintf("select * from ibf_topics left join xbt_files on bt_info_hash = info_hash where %s%s order by pinned desc, %s%s %s limit %d, %d", $query, $add_query, $topic_sort, $sort_key, $r_sort_by, $First, $ibforums->vars['display_max_topics']));
		}
