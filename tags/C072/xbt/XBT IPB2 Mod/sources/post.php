		...
		if ( $post_key AND ( $pid or $msg ) )
		{
			...
			if ( $cnt['cnt'] )
			{
				...
				else
				{
					...
					$DB->simple_exec();
					$DB->query("update ibf_topics t inner join ibf_posts p on t.tid = p.topic_id inner join ibf_attachments a on p.pid = a.attach_pid set t.bt_info_hash = a.bt_info_hash where a.bt_info_hash is not null");
				}
			}
		}
