<?php
	...
	function template_options($v)
	{
		...
		$d .= '<th>';
		$d .= sprintf('<tr><td align=left>Admin Port (TCP)<td><input type=text name=admin_port value=%d style="text-align: right"><td>', $v['admin port']['value']);
		$d .= sprintf('<tr><td align=left>Peer Port (TCP)<td><input type=text name=peer_port value=%d style="text-align: right"><td>', $v['peer port']['value']);
		$d .= sprintf('<tr><td align=left>Tracker Port (UDP)<td><input type=text name=tracker_port value=%d style="text-align: right"><td>', $v['tracker port']['value']);
		$d .= sprintf('<tr><td align=left>Upload Rate<td><input type=text name=upload_rate value=%d style="text-align: right"><td align=left>kb/s', $v['upload rate']['value'] >> 10);
