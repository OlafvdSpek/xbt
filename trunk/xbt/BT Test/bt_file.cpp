...
int Cbt_file::write_data(__int64 offset, const char* s, int cb_s, Cbt_peer_link* peer)
{
	...
	if (piece.write(offset % mcb_piece, s, cb_s))
	{
		if (offset % piece.cb_sub_piece())
			peer->alert(Calert::debug, "Piece " + n(a) + ", offset " + n(offset % mcb_piece) + ", size " + n(cb_s) + ": rejected");
		else
			peer->alert(Calert::debug, "Piece " + n(a) + ", chunk " + n(offset % mcb_piece / piece.cb_sub_piece()) + ", size " + n(cb_s) + ": rejected");
		return 1;
	}
	int size = cb_s;
	const char* r = s;
...
{
	int c = m_info.size() + m_name.size() + m_sub_files.size() + 8 * m_old_peers.size() + 49;
	for (t_trackers::const_iterator i = m_trackers.begin(); i != m_trackers.end(); i++)
		c += i->size() + 4;
	for (t_pieces::const_iterator i = m_pieces.begin(); i != m_pieces.end(); i++)
		c += i->pre_save_state();
