    ...
    function Boards()
    {
		...
		if ($ibforums->vars['show_totals'])
		{
			...
			if (1)
			{
				...
				$v = "";
				$v .= sprintf("%d announces (%d, %d, %d, %d)<br>", $result['announced'], $result['announced_http'], $result['announced_http_compact'], $result['announced_http_no_peer_id'], $result['announced_udp']);
				$v .= sprintf("%d scrapes (%d, %d)<br>", $result['scraped'], $result['scraped_http'], $result['scraped_udp']);
				$v .= sprintf("%d peers (100 %%)<br>", $result['peers']);
				if ($result['peers'])
				{
					$v .= sprintf("%d leechers (%d %%)<br>", $result['leechers'], $result['leechers'] * 100 / $result['peers']);
					$v .= sprintf("%d seeders (%d %%)<br>", $result['seeders'], $result['seeders'] * 100 / $result['peers']);
				}
				$v .= sprintf("%d torrents<br>", $result['torrents']);

				$stats_html .= $this->html->ShowTrackerStats($v);
			}
		}
